...
const AllVendors = () => {
    ...
    const fetchData = async (searchQuery) =>
    {
        try
        {
            axios.get(`${apiUrl}/user/getAllVendors`, {params: {searchQuery: searchQuery, page: page, pageSize: pageSize}})
            .then((res)=>
            {
    ...
    const vendor_columns_memo = useMemo(() => vendor_columns, []);
    const vendor_data = useMemo(() => (vendors || []), [vendors]);

    const vendor_table = useTable({columns: vendor_columns_memo, data: vendor_data});
    const {getTableProps, getTableBodyProps, headerGroups, rows, prepareRow} = vendor_table;
      ...
      <div className="p-4 sm:ml-64 bg-gray-100 dark:bg-gray-900 font-mono flex-grow">
        <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 min-h-screen w-full overflow-y-auto">
          ...
          <p className="text-gray-400 dark:text-white">
            These are all the vendors in the system database.
          </p>
          ...
          <div className="flex items-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
            ...
            <div>
              <span className="font-medium">No vendors found</span>
            </div>
          </div>
