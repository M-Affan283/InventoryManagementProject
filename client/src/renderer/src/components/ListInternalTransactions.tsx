...
const ListInternalTransactions = () => {
    ...
    const fetchData = async (searchQuery) =>
    {
        try
        {
            axios.get(`${apiUrl}/goods/getContractorWork`, {params: {email: user?.email ,searchQuery: searchQuery, page: page, pageSize: pageSize}})
            .then((res)=>
            {
    ...
    const transaction_columns = [
        {
          ...
          Cell: ({row}) => (
            <p>{row.original.contractor_id.contractor_name}</p>
          )
        },
        {
            Header: "Goods Weight",
        ...
        },
        {
          ...
          Cell: ({row}) => (
            <p className="">
                <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={()=>nav(`/individualinttransaction/${row.original._id}`)}>View</button>
            </p>
          )
        }
      ]
      ...
      <div className="p-4 sm:ml-64 bg-gray-100 dark:bg-gray-900 font-mono flex-grow">
        <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 min-h-screen w-full overflow-y-auto">
          ...
          <p className="text-2xl text-gray-400 dark:text-white">
            Internal Transactions (Contractor Work)
          </p>

          <p className="text-gray-400 dark:text-white">
            {user?.role === "admin" ? "These are all the internal transactions in system database." : "These are all the internal transactions you have added."}
          </p>
...
export default ListInternalTransactions