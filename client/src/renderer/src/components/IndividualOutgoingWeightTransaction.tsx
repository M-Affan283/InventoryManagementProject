...
const IndividualOutgoingWeightTransaction = (props:any) => {
        ...
        try
        {
            ...
            .then((res)=>
            {
                ...
                if(res.status === 200)
                {
                    ...
                    res.data.transaction.truck_no && setTruckNo(res.data.transaction.truck_no);
                    setType(res.data.transaction.type);
                    setContainerNo(res.data.transaction.container_no);
                    setDriverName(res.data.transaction.driver_name);
                    setDriverContact(res.data.transaction.driver_contact);
                    setContainerWeight(res.data.transaction.container_weight.toFixed(2));
    ...
    const calculateGoodsWeight = () => 
    {
        ...
        const goodsWeight = parseFloat(truckContainerGoodsWeight || '0') - parseFloat(truckWeight || '0') - parseFloat(containerWeight);
        setGoodsWeight(goodsWeight.toFixed(2).toString());
    ...
    const updateContainer = (e:any) =>
    {
        ...
        if(parseFloat(goodsWeight) < 0)
        {
            console.log("Goods weight cannot be negative");
            setServerResponse({message: "Goods weight cannot be negative", status: 400});
            closeForm();
            return;
        }
        ...
        try
        {
            ...
            .then((res)=>
            {
                if(res.status === 200)
                {
                    ...
                    setTimeout(()=>nav("/listoutgoingtransactions"), 2500);
                }
                else
                {
    ...
    <div className='flex flex-row w-full overflow-hidden dark:bg-gray-800 dark:text-white'>
     ...
     <div className="p-4 sm:ml-64 bg-gray-100 dark:bg-gray-900 font-mono flex-grow">
       <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 min-h-screen w-full overflow-y-auto">
         ...
         <button type="button" className="text-white inline-flex items-center bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 gap-3" onClick={()=>nav('/listoutgoingtransactions')}>
            <ArrowBackIcon/>
            Back
        </button>
        ...
        {transaction ? 
            ...
            <div className="py-8 px-4 mx-auto max-w-2xl lg:py-16">
                ...
                <dl className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 text-lg">
                  ...
                  <div>
                    <dt className="mb-2 font-semibold leading-none text-gray-900 dark:text-white">Container No:</dt>
                    <dd className="mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-400">{transaction.container_no}</dd>
                  </div>
                  <div>
                  ...
                  <div>
                    <dt className="mb-2 font-semibold leading-none text-gray-900 dark:text-white">Outgoing Container Weight:</dt>
                    <dd className="mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-400">{parseFloat(transaction.container_weight).toFixed(2)}</dd>
                  </div>
                  <div>
                  ...
                  <div>
                    <dt className="mb-2 font-semibold leading-none text-gray-900 dark:text-white">Date of Container Arrival:</dt>
                  ...
                  { transaction.date_filled_weight &&
                    ...
                    <div>
                        <dt className="mb-2 font-semibold leading-none text-gray-900 dark:text-white">Date of Container Dispatch:</dt>
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <button type="button" className="inline-flex items-center text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" onClick={()=>nav(`/genoutgoingweightpdf/${transaction_id}`)}>
                        <DocumentScanner/>
                        Generate PDF
                    </button> 
        ...
        {updateBool && props.desktopApp &&
            ...
            {/* make this for wider */}
            <form className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full dark:text-white" onSubmit={updateContainer}>
                    ...
                    <div className="relative z-0 w-full mb-5 group flex flex-col">
                        <input type="number" name="empty_weight" id="floating_first_name" className="block py-2.5 px-0 w-full text-lg text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" value={truckWeight} required onChange={(e)=>{setTruckWeight(e.target.value);}} />
                        <label className="peer-focus:font-medium absolute text-base text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Transporting Truck Weight</label>
                    ...
                    <div className="relative z-0 w-full mb-5 group flex flex-col">
                        <input type="number" name="empty_weight" id="floating_first_name" className="block py-2.5 px-0 w-full text-lg text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" value={truckContainerGoodsWeight} required onChange={(e)=>{setTruckContainerGoodsWeight(e.target.value);}} />
...
export default IndividualOutgoingWeightTransaction