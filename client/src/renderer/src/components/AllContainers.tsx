...
const AllContainers = () => {
  ...
  const fetchData = async (searchQuery) =>
  {
    try
    {
      axios.get(`${apiUrl}/goods/getContainers`, {params: {email: user?.email ,searchQuery: searchQuery, page: page, pageSize: pageSize}})
      .then((res)=>
      {
        console.log("Containers fetched successfully");
        setContainers(res.data.containers);

      })
      .catch((err)=>
      {
        console.log("Failed to fetch containers: ", err.response.data.message);
        setServerResponse("Error fetching containers");
      })
  ...
  const container_columns = [
    ...
    },
    {
      ...
      Cell: ({row}) => (
        <p className="">
            <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={()=>nav(`/individualcontainer/${row.original._id}`)}>View</button>
        </p>
      )
    }
  ]

  const container_columns_memo = useMemo(() => container_columns, []);
  const container_memo = useMemo(() => (containers || []), [containers]);

  const container_table = useTable({columns: container_columns_memo, data: container_memo});

  const { getTableProps, getTableBodyProps, headerGroups, rows, prepareRow } = container_table;
      ...
      <div className="p-4 sm:ml-64 bg-gray-100 dark:bg-gray-900 font-mono flex-grow">
        <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 min-h-screen w-full overflow-y-auto">
          ...
          <p className="text-gray-400 dark:text-white">
            {user?.role === "admin" ? "These are all the trucks in system database." : "These are all the trucks you have added."}
          </p>
